$(document).ready(function() {
  ...
  loginSubmit.click(function() {
    //이메일 유효성 검사식이다. 이 식은 정규식이라고 한다.
    //PHP에서 학습하므로 그냥 따라하자.
    var regEmailPattern = /^[a-zA-Z_\-0-9]+@[a-z]+.[a-z]+$/;
    //아래 if문의 조건문은 위의 정규식을 이메일 주소가 따르는지 아닌지를 판별한다.
    //앞에 !가 있는 것에 주의할것.
  ...
  //회원가입 입력 정보 필터링 - 회원가입 폼에 입력한 값이 올바른지 필터링하는 소스.
  ...
  signUpSubmit.click(function() {
    ...
    //아래의 식은 PHP 학습 시 학습하므로 그냥 따라하자.
    //입력한 값이 한글이거나 알파벳이라는 것을 확인하는 식이다.
    ...
    $.ajax({
      ...
      //valueCheck.js를 index.php에서 불러와서 사용하는거라서, 폴더 이동 기준(./이냐 ../이냐가)이 index.php를 기준으로 해야됨? - 근데 확실하지 않음.
